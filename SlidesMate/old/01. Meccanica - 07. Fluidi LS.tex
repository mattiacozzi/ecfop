...
\begin{center}
\colorbox{blue!30}{$ p = \dfrac{F}{S} $} ~~~~~~ $ [ Pa ] = \left[ \dfrac{N}{m^2} \right] $
\end{center}
...
\begin{center}
$ p_1 = p_2 $~~~\pause$ \Longrightarrow  $~~~$ \dfrac{F_1}{A_1} = \dfrac{F_2}{A_2} $\pause~~~$ \Longrightarrow  $~~~\colorbox{blue!30}{$ F_1 = \dfrac{A_1}{A_2}F_2 $}
\end{center}
...
\begin{center}
\colorbox{blue!30}{$ p_0 = 1 \, atm = 101\, 325 \, Pa \sim 1,01 \times 10^5 \, Pa $}
\end{center}
\end{frame}
...
\begin{center}
\colorbox{blue!30}{$ p = d g h $} ~~~~~~ Legge di Stevino
\end{center}\pause
Se consideriamo anche la pressione atmosferica che preme sul liquido:
\begin{center}
\colorbox{blue!30}{$ p = d g h + p_0 $}
\end{center}
\end{frame}


\begin{frame}
\frametitle{Dimostrazione della legge di Stevino}
Immaginiamo un punto al fondo di un recipiente alto $ h $ e di base $ B $.\pause

~

La forza premente in quel punto è la forza peso $ mg $, dove $ m $ è la massa di liquido sopra al punto in questione.\pause

~

Sapendo che $ m = dV $ e considerando che $ V = Bh $, otteniamo:
\begin{center}
  $ p = \dfrac{F}{S} = \dfrac{mg}{B} = \dfrac{dVg}{B} = \dfrac{dBhg}{B} = dgh $
\end{center}
\begin{figure}
  \includegraphics[width=.3\columnwidth]{img/stevino.jpg}
  \end{figure}
\end{frame}
...
\begin{exampleblock}{Calcolo della forza}
  Un sottomarino si trova a $ 14 \, m $ di profondità in un bacino di acqua dolce. Calcola la forza che agisce su un oblò del sottomarino sapendo che il diametro dell'oblò è di $ 20 \, cm $.
  
  Ricorda che sull'oblò agiscono due pressioni, una esterna e una interna (pari a $ 1 \, atm $).\hspace*{\fill}[$ 4,3 \times 10^3 \, N $]
\end{exampleblock}
\end{frame}
...
\end{figure}
Possiamo intuire che \alert{anche la spinta di Archimede è una conseguenza della legge di Stevino}.
\end{frame}
...
\end{center}\pause
Poiché $ m_{fluido} = d_{fluido}  V_{fluido} $ allora:
\begin{center}
$ S = d_{fluido}  V_{fluido}  g $
\end{center}\pause
Tuttavia, $ V_{fluido} = V_{immerso} $, e pertanto:
\begin{center}
\colorbox{blue!30}{$ S = d_{fluido}  V_{immerso}  g $}
\end{center}
\end{frame}
...
\end{center}\pause
\begin{center}
$ d_{f}  V_{c}  \cancel{g} \geq m_{c} \cancel{g} $
\end{center}\pause
\begin{center}
$ d_{f} \cancel{V_{c}} \geq d_{c} \cancel{V_{c}} $
\end{center}\pause
\begin{center}
\colorbox{blue!30}{$ d_{f}  \geq d_{c}  $}
\end{center}\pause
Se $ d_{f} < d_{c}  $, il corpo affonda (ma avrà un \emph{peso relativo} minore).
\end{frame}
...
\begin{frame}
\frametitle{Esercizi}
\begin{exampleblock}{Calcolo della densità}
  Un corpo in sughero galleggia sull'acqua. Sapendo che i $ 4/5 $ del suo volume sono fuori dal pelo dell'acqua, determina la densità del sughero.\hspace*{\fill}[$ 2,00 \times 10^2 \, N $]
\end{exampleblock}\pause

~

\begin{exampleblock}{Volume emerso}
  Un blocco di ghiaccio ($ d_g = 9,20 \times 10^{2} \, kg/m^3 $) a forma di parallelepipedo di altezza pari a $ 30 \, m $ galleggia in acqua di mare ($ d_a = 1,03 \times 10^{3} \, kg/m^3 $).
  
  Quanto sarà lunga la parte del parallelepipedo al di fuori dell'acqua?\hspace*{\fill}[$ 3,2 \, m $]
\end{exampleblock}
\end{frame}
